...
def clean_to_List(d:list, url:str) -> list:
    ...
    return [inner_clean(i) for i in new_d]
'''
def merge_flatten(d:list) -> typing.List:
    return d if all(not isinstance(i, list) for i in d) else d[0]


def can_merge(d:typing.List[typing.Any]) -> bool:
    if all(isinstance(i, list) for i in d):
        return len(d) == 1 and len(d[0]) < 4 and all(not isinstance(i, list) for i in d[0])
    return len(d) < 4 and all(not isinstance(i, list) for i in d)
...
def clean_to_List(d:list, url:str) -> list:
    ...
    def inner_clean(_d, to_table = True, parent=0, table=0):
        if to_table:
            ...
            def table_formation(row):
                ...
                for i in row:
                    freq[not isinstance(i, list)] += 1
                if freq[True] >= freq[False]:
                    #return list(filter(None, [filter_row([k.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) if not isinstance(k, list) else inner_clean(k, to_table=True, parent = new_table) for k in row])])) #old way
                    return list(filter(None, [filter_row([i for k in row for i in ([k.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url)] if not isinstance(k, list) else ([x.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) for x in merge_flatten(k)] if can_merge(k) else [inner_clean(k, to_table=True, parent = new_table)]))])]))
                new_result = []
                for k in row:
                    if not isinstance(k, list):
                        ...
                        if sum(isinstance(i, list) for i in k) > sum(not isinstance(i, list) for i in k):
                            ...
                            #new_result.append(list(filter(None, filter_row([i.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) if not isinstance(i, list) else inner_clean(i, to_table=True, parent=new_table) for i in k])))) #old way
                            new_result.append(list(filter(None, filter_row([j for i in k for j in ([i.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url)] if not isinstance(i, list) else ([x.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) for x in merge_flatten(i)] if can_merge(i) else [inner_clean(i, to_table=True, parent=new_table)]))]))))
                        #new_result.append([inner_clean(k, to_table=True, parent=new_table)])
                    else:
        ...
        #return filter_row([k.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) if not isinstance(k, list) else inner_clean(k, to_table=True, table=parent, parent = parent) for k in _d]) #old way
        return filter_row([i for k in _d for i in ([k.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url)] if not isinstance(k, list) else ([x.format_payload(table=table, parent=parent, forbidden=_forbidden, url=url) for x in merge_flatten(k)] if can_merge(k) else [inner_clean(k, to_table=True, table=parent, parent = parent)]))])
