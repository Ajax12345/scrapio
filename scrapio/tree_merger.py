...
class LookupPath:
    ...
    def generate_path(cls, _main, *targets) -> typing.Any:
        ...
        #print([[i[-1][-1], cls.lookup_path(i, a) for a in targets] for i in _result])
...
class _basic_attrs(_elem_attrs):
    def __new_eq_class(self, _obj) -> bool:
        if getattr(_obj, 'is_stylestring', False):
            return False
        '''
        _result = all((lambda x, y:x == y if not (isinstance(x, list) and isinstance(y, list)) else bool(set(x)&set(y)))(self(i), _obj(i)) for i in ['tag', 'class'])
        print([[self(i), _obj(i)] for i in ['tag', 'class']], _result)
        return _result
        '''
...
class StyleTree(tree_merge_utilites.TreeAttrs):
    def __init__(self, _main:bs4.BeautifulSoup, _matched:typing.List[bs4.BeautifulSoup]) -> None:
        ...
        #self.contents = [i for i in self.__class__.merge_children(self.main.contents, *[i.contents for i in self.matched]) if not getattr(i, 'is_stylestring', False) or i.is_empty_string]
    ...
    def prune_tree(self) -> None:
        ...
        #self.contents = [i for i in self.contents if not isinstance(i, list) or i]
